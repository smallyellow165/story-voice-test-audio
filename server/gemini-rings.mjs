// Ring business logic retained from ring-llm.mjs; only provider/configuration changes.
...
export function geminiRingModelOptions() {
  const defaultModel = process.env.GEMINI_RING_MODEL || 'gemini-3.6-flash'
  return { apiKeyConfigured: Boolean(process.env.GEMINI_API_KEY?.trim()), defaultModel, models: models.some(m => m.id === defaultModel) ? models : [...models, { id: defaultModel, label: `${defaultModel} (server default)` }] }
}
...
export async function detectRingsWithGemini(body) {
  ...
  try {
    data = await ai.models.generateContent({
      ...
      config: { ...(model === 'gemini-2.5-flash' ? { temperature: 0 } : {}), maxOutputTokens: 8192, responseMimeType: 'application/json', responseSchema: ringSchema },
    })
  } catch (error) {
    // Never return SDK request/config, credentials, or the submitted image.
    const status = Number.isInteger(error?.status) ? error.status : undefined
    throw Object.assign(new Error(`Gemini Developer API request failed for ${model}${status ? ` (HTTP ${status})` : ''}. Check GEMINI_API_KEY, model availability, quota and network connectivity.`), { statusCode: 502 })
  }
  ...
  try {
    ...
    return { model, modelVersion: data.modelVersion, ...result, rawJson }
  } catch (error) {
